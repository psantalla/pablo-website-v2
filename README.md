...
**Video posters**: `scripts/video-posters.js` generates a sibling `.webp` for every `content/**/*.mp4` (first frame, via `ffmpeg`). Runs on `prestart`/`prebuild`, so local dev and the GitHub Action regenerate them automatically; posters are gitignored. To move this to another host or machine: ensure `ffmpeg` is on PATH (CI: any `ubuntu-*` runner already has it; local: `brew install ffmpeg`).
